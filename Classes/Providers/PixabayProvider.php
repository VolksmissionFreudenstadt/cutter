...
class PixabayProvider extends AbstractProvider
{
    ...
    public function retrieveImage($imageUrl)
    {
        ...
        $meta['description'] = '';
        $tmp = $pDoc->find('div.right div.clearfix a');
        foreach ($tmp as $n) {
            $author = trim(explode('/', $n->textContent)[0]);
        }
        $meta['author'] = explode(' / ', $author)[0];
        $meta['license'] = 'CC0 Public Domain, https://creativecommons.org/publicdomain/zero/1.0/deed.de';


        $links = $pDoc->find('div.download_menu table input');
        $basePath = '';
        foreach ($links as $link) {
            $meta['src'][] = $this->configuration['baseUrl'] . pathinfo($link->getAttribute('value'), PATHINFO_BASENAME);
        }


        $path = str_replace('/en/', '', parse_url($imageUrl, PHP_URL_PATH));
        $tmp = explode('-', $path);
    ...
    {
        $res = $this->post('https://pixabay.com/en/accounts/login/', array(
